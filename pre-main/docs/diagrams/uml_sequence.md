...
Диаграмма последовательности описывает основной сценарий прохода сотрудника через КПП.
...
- Низкая уверенность не приводит к автоматическому открытию турникета.
- События с подозрением на spoofing фиксируются как инциденты ИБ.
- Ручное решение оператора также записывается в audit log и используется для последующего анализа качества.
